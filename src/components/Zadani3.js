...
function Zadani3() {
  ...
  const Data = () => {
    const [state, setState] = useState(
      GetStorage("data") || data.sort((a, b) => a.id - b.id)
    )
    const [edit, setEdit] = useState({ show: false, id: -1 })
    const [select, setSelect] = useState({ show: false, id: -1 })
    console.log(edit)
            ...
            return (
              <TableRow>
                ...
                {columns.campaign_name && (
                  <TableCell
                    onClick={() => {
                      setEdit({ show: true, id: element.id })
                    }}
                  >
                    {edit.show && edit.id === element.id ? (
                      <>
                        <TextField
                          value={state[index].campaign_name}
                          onChange={(e) => {
                            let copyState = [...state]
                            copyState[index].campaign_name = e.target.value
                            setState([...copyState])
                            SetStorage("data", [...copyState])
                          }}
                          onKeyDown={(event) => {
                            if (event.key === "Enter") {
                              setEdit({ show: false })
                ...
                {columns.adwords_status && (
                  <TableCell
                    onClick={() => {
                      setSelect({ show: true, id: element.id })
                    }}
                  >
                    {select.show && element.id === select.id ? (
                      <NativeSelect
                        className="select"
                        value={element.adwords_status}
                        onChange={(e) => {
                          let copyState = [...state]
                          copyState[index].adwords_status = e.target.value
                          setState([...copyState])
                          SetStorage("data", [...copyState])
